...
class FirebaseUtils {
  ...
  // static Future<List<TaskModel>> readTask(DateTime selectedTime) async {
  //   // reference the collection and query || create if !exist and query
  //   var collectionRef = getCollection()
  //       .where("selectedDate", isEqualTo: selectedTime.millisecondsSinceEpoch);
  //
  //   // get documents
  //   var data = await collectionRef.get();
  //
  //   // get the document data
  //   var tasksList = data.docs
  //       .map(
  ...
  static Future<void> updateTask(TaskModel task) async {
    ...
    var documentRef = collectionRef.doc(task.id);
    // delete the document
    return documentRef.update({"isDone": true});
  }
}
